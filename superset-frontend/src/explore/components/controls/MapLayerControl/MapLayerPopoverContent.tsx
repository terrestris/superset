...
const MapLayerActionsContainer = styled.div`
...
interface MapLayerPopoverContentProps {
  onClose?: () => void;
  onSave?: (layerConf: LayerConf) => void;
  layerConf: LayerConf;
}

export const LayerConfigsPopoverContent: React.FC<MapLayerPopoverContentProps> =
  ({ onClose = () => {}, onSave = () => {}, layerConf }) => {
    ...
    const isValidLayerConf = (conf: LayerConf) => {
      const hasUrl = !!conf.url;
      const hasTitle = !!conf.title;

      if (!hasUrl || !hasTitle) {
        return false;
      }
      if (isWfsLayerConf(conf)) {
        const hasWfsVersion = wfsVersion !== undefined;
        const hasTypeName = conf.typeName !== undefined;
        return hasWfsVersion && hasTypeName;
      }
      if (isWmsLayerConf(conf)) {
        const hasWmsVersion = wmsVersion !== undefined;
        const hasLayersParam = conf.layersParam !== undefined;
        return hasWmsVersion && hasLayersParam;
      }
      if (isXyzLayerConf(conf)) {
        return true;
      }
    ...
    const onLayerTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      onFieldValueChange(e.target.value, 'title');
    };

    const onLayerUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      onFieldValueChange(e.target.value, 'url');
    };

    const onLayersParamChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      onFieldValueChange(e.target.value, 'layersParam');
    };

    const onTypeNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      onFieldValueChange(e.target.value, 'typeName');
    };
    ...
    const onAttributionChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      onFieldValueChange(e.target.value, 'attribution');
    };
    ...
    return (
      <div>
        ...
        <MapLayerActionsContainer>
          <Button buttonSize="small" onClick={onCloseClick} cta>
            {t('Close')}
          </Button>
          <Button
            disabled={!isValidLayerConf(currentLayerConf)}
            buttonStyle="primary"
            buttonSize="small"
            className="m-r-5"
            onClick={onSaveClick}

...
import { ExpMapChartSize, MapChartSize } from '@superset-ui/chart-controls';
import {
  computeConfigValues,
  MAX_ZOOM_LEVEL,
  MIN_ZOOM_LEVEL,
...
describe('computeConfigValues', () => {
  it('computes fixed values', () => {
    ...
    const zoomConfigs: MapChartSize = {
      type: 'FIXED',
      values: {},
      configs: {
        zoom: 2,
...
describe('toFixedConfig', () => {
  const configs: MapChartSize['configs'] = {
    width: 100,
    height: 100,
    zoom: 5,
  };
...
describe('toExpConfig', () => {
  const configs: ExpMapChartSize['configs'] = {
    width: 100,
    height: 100,
    zoom: 5,
    exponent: 1.5,

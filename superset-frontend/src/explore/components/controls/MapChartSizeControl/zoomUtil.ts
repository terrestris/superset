 ...
 */
export const zoomConfigsToData = (zoomConfigs: MapChartSize['values']) =>
 ...
 */
export const dataToZoomConfigs = (data: number[][]): MapChartSize['values'] =>
  data.reduce((prev: MapChartSize['values'], cur: number[]) => {
 ...
 */
const computeFixedConfigValues = (zoomConfigsFixed: FixedMapChartSize) => {
  const values: FixedMapChartSize['values'] = {};
 ...
 * @returns The computed values for each zoom level.
 */
const computeLinearConfigValues = (zoomConfigsLinear: LinearMapChartSize) => {
  const values: LinearMapChartSize['values'] = {};
  for (let i = MIN_ZOOM_LEVEL; i <= MAX_ZOOM_LEVEL; i += 1) {
    const aspectRatio =
 ...
 * @returns The computed values for each zoom level.
 */
const computeExpConfigValues = (zoomConfigsExp: ExpMapChartSize) => {
  const values: ExpMapChartSize['values'] = {};
  const x = Math.pow(
 ...
 *
 * @param mapChartSize The config to base the computation upon.
 * @returns The computed values for each zoom level.
 */
export const computeConfigValues = (mapChartSize: MapChartSize) => {
  if (isFixedMapChartSize(mapChartSize)) {
    return computeFixedConfigValues(mapChartSize);
  }
  if (isLinearMapChartSize(mapChartSize)) {
    return computeLinearConfigValues(mapChartSize);
  }
  return computeExpConfigValues(mapChartSize);
};
...
export const toFixedConfig = (
  baseConfig: FixedMapChartSize['configs'],
): FixedMapChartSize => {
  const zoomConfigFixed: FixedMapChartSize = {
    type: 'FIXED',
    configs: {
...
export const toLinearConfig = (
  ...
  const zoomConfigsLinear: LinearMapChartSize = {
    type: 'LINEAR',
    configs: {
      zoom: baseConfig.zoom,
      width: baseConfig.width,
...
export const toExpConfig = (
  baseConfig: ExpMapChartSize['configs'],
): ExpMapChartSize => {
  const zoomConfigExp: ExpMapChartSize = {
    type: 'EXP',
    configs: {
      zoom: baseConfig.zoom,
      width: baseConfig.width,

...
case "${1}" in
  ...
  app)
    ...
    if [ "$DEBUGGER" = "1" ]; then
        ...
        python -m debugpy --listen 0.0.0.0:5678 -m flask run -p $PORT --reload --debugger --without-threads --host=0.0.0.0 --exclude-patterns "*/node_modules/*:*/.venv/*:*/build/*:*/__pycache__/*"
    else
